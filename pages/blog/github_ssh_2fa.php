...
<p>
  ...
  <a href="https://www.raspberrypi.org/blog/raspberry-pi-zero/"
    >raspberry pi zero</a
  >
  as a dedicated password manager. It could more accurately be described as a
  secrets keeper/manager, but I use it mostly for passwords.<br />
  Anyway, I wanted to set up 2FA for as many services as I could (I'm currently
  using
  <a href="https://f-droid.org/en/packages/org.liberty.android.freeotpplus/"
    >FreeOTP+</a
  >
  on android and it has everything I need/want), so I started setting them up.
  Both my raspberry pi and my android have the OTPs for redundancy.
</p>
...
<p>
  ...
  be accepted. You'll get an "invalid username or password" error message. This
  happens because GitHub uses the <b>https protocol</b> by default and
  recommends it for its users, because it usually works <i>out of the box</i>.
</p>
...
<p>
  ...
  passwords (more vulnerable) and this new <i>"token password"</i> defeats the
  purpose of 2FA (since it bypasses the 2FA temporary code). You can mitigate
  these problems by limiting how much <i>power</i> the access token has (give it
  fewer permissions), but it still isn't optimal.
</p>
...
<p>
  ...
  <a href="https://happygitwithr.com/ssh-keys.html">tutorial</a>, which helped
  me understand the subject better and quickly set everything up correctly.
</p>
...
<p>
  ...
  settings, access the <b>SSH and GPG keys</b> tab, and copy the contents of
  your public key to a new <b>ssh key</b>, e.g. using:
  <code>xclip -sel clip &lt; ~/.ssh/id_rsa.pub</code> to insert the key contents
  into your clipboard.
</p>

<p>
  Now that you have associated your <b>ssh key</b> with your account, you just
  need to start using the <b>ssh protocol</b> versions of your remote links.
</p>
...
<p>
  ...
  <code>git@github.com:leereilly/games.git</code>. As you can see, it is really
  easy to convert from one <b>protocol</b> to another. I even found a
  <a href="https://gist.github.com/m14t/3056747">small bash script</a> to make
  the conversion of your repository's remotes to the <b>ssh protocol</b>.
</p>
...
<p>
  ...
  while using the <b>ssh protocol</b> is having the connection to port 22 (ssh
  port) blocked is in your network. I've never had that problem, but GitHub
  suggests that you may still be able to
  <a
