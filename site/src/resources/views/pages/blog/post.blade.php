...
<div class="w-full">
  <article class="m-auto relative prose blog line-numbers match-braces">
    @auth
    <a class="absolute top-0 right-0 z-50 icon-btn btn-edit" title="Edit post {{ $b->blog_id }}" href="{{ route('blog_edit', ['id' => $b->blog_id]) }}">
      <i class="fa-solid fa-pen-to-square"></i>
    </a>
    @endauth

    <h1 class="mb-0">{!! Parsedown::instance()->line($b->blog_title) !!}</h1>
    <em class="block muted mb-4">Avg. {{ $b->readingTime() }} minute(s) of reading</em>

    @if (!is_null($b->blog_intro))
    <div class="p-2 rounded-md bg-background-300 dark:bg-background-900">
      {!! Parsedown::instance()->text($b->blog_intro) !!}
    </div>
    @endif

    {!! Parsedown::instance()->text($b->blog_content) !!}
  </article>
</div>
